...
* Git repository ([URL](configuration.md#repositoryurl) and options [release branches](configuration.md#branches) and [tag format](configuration.md#tagformat))
* Plugins [declaration](configuration.md#plugins) and options
* Run mode ([debug](configuration.md#debug), [dry run](configuration.md#dryrun) and [local (no CI)](configuration.md#ci))
...
Additionally, metadata of Git tags generated by **semantic-release** can be customized via standard [Git environment variables](configuration.md#git-environment-variables).

## Configuration file

**semantic-release**’s [options](configuration.md#options), mode and [plugins](plugins.md) can be set via either:
