...
2. [Choose a publishing path](#choose-a-publishing-path).
   ...
   - If you cannot use trusted publishing, add `NPM_TOKEN` as a GitHub Actions secret as described in [Environment variables](#environment-variables) and follow [Path B](#path-b-fallback-npm-token).
