...
All of these options can be configured through config file, CLI arguments or by extending a [shareable configuration](/usage/shareable-configurations).
...
**semantic-release**’s [options](#options), mode and [plugins](/usage/plugins) can be set via either:
...
List of modules or file paths containing a [shareable configuration](/usage/shareable-configurations). If multiple shareable configurations are set, they will be imported in the order defined with each configuration option taking precedence over the options defined in a previous shareable configuration.
